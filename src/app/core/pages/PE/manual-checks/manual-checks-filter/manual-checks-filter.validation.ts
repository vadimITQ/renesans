...
export function validateFilter(filter: ISearchPaymentFilters): { success: boolean; validationMessage: ValidationMessage } {
  ...
  const filterHasDates: boolean =
    !!filter.dateTimeFrom && !!filter.dateTimeTo;
  ...
  if (filterHasDates) {
    const [dateFrom, dateTo] = [filter.dateTimeFrom, filter.dateTimeTo
      ];
...
export function validateDates(dateFrom: string | null, dateTo: string | null): Validation {
  ...
  const [_dateFrom, _dateTo] = [DatePickerHelper.convertToDate(dateFrom), DatePickerHelper.convertToDate(dateTo)];
